...
public class Grid
{
    ...
    private int targetScore = mMyAppsBundle.getInt("level") * 300;
    private int STARTMOVES = mMyAppsBundle.getInt("level") * 3;
    private int moves = STARTMOVES;
    private int score = 0;
    private int lastScore = 0;

    private boolean isReadyGameOver = false;
    ...
    public void highLightSelected(int TouchX,int TouchY)
    {
        int r1=0,c1=0;
        for (int r = 0; r < ROWS; r++)
        {
            for (int c = 0; c < COLS; c++)
            {
                if(TouchX >= c*FOODPIECEWIDTH + SCREENWIDTHOFFSET && TouchX < (c+1)*FOODPIECEWIDTH + SCREENWIDTHOFFSET)
                {
                    c1=c;
                }
                if(TouchY >= r*FOODPIECEHEIGHT + SCREENHEIGHTOFFSET && TouchY < (r+1) *FOODPIECEHEIGHT + SCREENHEIGHTOFFSET)
                {
                    r1=r;
                }
            }
        }
        foodPieces[r1][c1].setM_IsHighLight(true);
    }
    ...
    public void update(int firstTouchX,int firstTouchY, int secondTouchX, int secondTouchY)
    {
        if((Math.abs(firstTouchX - secondTouchX) <= FOODPIECEWIDTH + SCREENWIDTHOFFSET) && (Math.abs(firstTouchY - secondTouchY) <= FOODPIECEHEIGHT + SCREENHEIGHTOFFSET))
        {
            ...
            if(getSwapable(r1,c1,r2,c2))
            {
                ...
                foodPieces[r1][c1].setM_IsHighLight(false);
                foodPieces[r2][c2].setM_IsHighLight(false);
    ...
    public void checkMatches()
    {
        ...
        if(isReadyGameOver && !isAddingScore && !isGameLevelComplete)
        {
            isGameOver = true;
        }
    }
